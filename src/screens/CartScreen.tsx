      ...
      <Modal visible={showPaymentModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {selectedMethod === 'fib' && (
              <View style={{ backgroundColor: '#e3f2fd', borderRadius: 12, padding: 16, marginTop: 8 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <Ionicons name="business-outline" size={20} color="#1565C0" />
                  <Text style={{ fontSize: 14, fontWeight: 'bold', color: '#1565C0' }}>FIB - {t('payment.apiRequired')}</Text>
                </View>
                <Text style={{ fontSize: 12, color: '#666', textAlign: 'center' }}>
                  {t('payment.apiRequiredDesc')}
                </Text>
                <View style={{ marginTop: 6 }}>
                  <Text style={{ fontSize: 11, color: '#999' }}>• {t('payment.fibReq1')}</Text>
                  <Text style={{ fontSize: 11, color: '#999' }}>• {t('payment.fibReq2')}</Text>
                  <Text style={{ fontSize: 11, color: '#999' }}>• {t('payment.fibReq3')}</Text>
                </View>
              </View>
            )}
